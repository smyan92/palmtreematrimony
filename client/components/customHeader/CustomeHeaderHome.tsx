...
export default function CustomHeader() {
    ...
    // 🚨 1. DEFINE API BASE URL (Must be replaced with your actual server URL)
    const API_BASE_URL = 'http://192.168.43.38:5000';
    
const relativePhotoUrl = useSelector((state: RootState) => state.auth.user?.profilePhotoUrl);

// Use a conditional check or string coercion to safely display the value.
const photoUrlDebug = relativePhotoUrl 
    ? relativePhotoUrl 
    : (relativePhotoUrl === null ? "Value is NULL" : "Value is UNDEFINED/MISSING");

Alert.alert("Profile Photo URL", photoUrlDebug);
    
    // 3. Determine the image source based on the Redux path
    const LOCAL_ASSET_PATH_STRING = '/assets/images/logo.png';
    
    // imageSource will be either a local resource ID (number) or a remote URI object ({uri: string})
    let imageSource: any; 

    if (relativePhotoUrl === LOCAL_ASSET_PATH_STRING) {
        // Case A: Redux path explicitly matches the local asset string. Use the imported module.
        imageSource = logo; 
    } else if (relativePhotoUrl) {
        // Case B: Redux holds a dynamic path for a user's remote photo. Construct the full URL.
        const fullRemoteUrl = `${API_BASE_URL}${relativePhotoUrl.startsWith('/') ? '' : '/'}${relativePhotoUrl}`;
        imageSource = { uri: fullRemoteUrl };
    } else {
        // Case C: Redux path is null/undefined. Use the local logo as a default placeholder.
        imageSource = logo;
    }
        ...
        <View>
            ...
            <View style={styles.header}>
                <View style={styles.leftContainer}>
                    {/* The Image component uses the calculated imageSource directly */}
            ...
            <ProfileCompletionModal
                ...
                // Removed the unnecessary onPress that opened the modal from the header
            />
        </View>
    );
}
